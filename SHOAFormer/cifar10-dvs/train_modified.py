...
def parse_args():
    ...
    parser.add_argument('--data-path', default='/home/ps/Dataset/CiFar/DVS/', help='dataset')
...
def plot_metric_curves(history, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'loss_curve.eps'), format='eps')
    plt.close(fig)

    fig = plt.figure(figsize=(7, 5))
    plt.plot(epochs, history['train_acc1'], label='Train Acc@1')
    plt.plot(epochs, history['test_acc1'], label='Test Acc@1')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.title('Accuracy Curve')
    plt.legend(framealpha=1.0)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'accuracy_curve.eps'), format='eps')
    plt.close(fig)
...
def collect_predictions(model, data_loader, device):
    ...
    return torch.cat(all_targets).numpy().astype(np.int64), torch.cat(all_preds).numpy().astype(np.int64)


def build_confusion_matrix(targets, preds, num_classes):
...
def save_confusion_matrix_eps(cm, output_path, class_names=None):
...
def main(args):
    ...
    if args.test_only:
        ...
        targets, preds = collect_predictions(model, data_loader_test, device)
        cm = build_confusion_matrix(targets, preds, args.num_classes)
        save_confusion_matrix_eps(cm, os.path.join(output_dir, 'confusion_matrix.eps'))
        return
    ...
    for epoch in range(args.start_epoch, num_epochs):
        ...
        history['test_acc1'].append(float(test_acc1))
    ...
    if output_dir and checkpoint is not None:
        utils.save_on_master(
            checkpoint,
            os.path.join(output_dir, f'checkpoint_{epoch}.pth'))

    if output_dir and utils.is_main_process():
        plot_metric_curves(history, output_dir)

        current_state = copy.deepcopy(model_without_ddp.state_dict())
        if best_model_state is not None:
            model_without_ddp.load_state_dict(best_model_state)
        targets, preds = collect_predictions(model, data_loader_test, device)
        cm = build_confusion_matrix(targets, preds, args.num_classes)
        save_confusion_matrix_eps(cm, os.path.join(output_dir, 'confusion_matrix.eps'))
        model_without_ddp.load_state_dict(current_state)
