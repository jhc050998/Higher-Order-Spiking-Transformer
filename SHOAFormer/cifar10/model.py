...
class Spiking_Higher_Order_Self_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.k1_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1,bias=False)
        self.k1_bn = nn.BatchNorm1d(dim)
        self.k1_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')
        
        self.k2_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1,bias=False)
        self.k2_bn = nn.BatchNorm1d(dim)
        self.k2_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='torch')
    ...
    def derive_high_order_k(self, k1, k2):  # k1/k2 - (4, 64, 8, 64, 48)
        dim = -1  # *** roll dim
        n = k2.size(dim)  # qb len on dim
        
        k2_perm = k2.movedim(dim, -1)  # push target dim to the last one [..., n]
        base = torch.arange(n, device=k2.device)
        idx = (base[:, None] + base[None, :]) % n   # [n, n]
        view_shape = [1] * (k2_perm.ndim - 1) + [n, n]  # expand to broadcast shape: [1, 1, ..., n, n]
        idx = idx.view(*view_shape)
        k2_expand = k2_perm.unsqueeze(-2).expand(*k2_perm.shape[:-1], n, n)  # expand qb to [..., 1, n], then gather to [..., n, n]
        idx = idx.expand(*k2_perm.shape[:-1], n, n)
        ex_k2 = torch.gather(k2_expand, dim=-1, index=idx)   # [..., n, n]
        ex_k2 = ex_k2.movedim(-2, dim)
        
        k = torch.matmul(ex_k2, k1.unsqueeze(-1)).squeeze(-1)
        return k
        

    def forward(self, x):
        T, B, C, H, W = x.shape
        #print("Input x shape: " + str(x.shape))  # (4, 64, 384, 8, 8)
        ...
        k1_conv_out = self.k1_conv(x_for_qkv)
        k1_conv_out = self.k1_bn(k1_conv_out).reshape(T,B,C,N).contiguous()
        k1_conv_out = self.k1_lif(k1_conv_out)
        k1 = k1_conv_out.transpose(-1, -2).reshape(T, B, N, self.num_heads, C//self.num_heads).permute(0, 1, 3, 2, 4).contiguous()
        
        k2_conv_out = self.k2_conv(x_for_qkv)
        k2_conv_out = self.k2_bn(k2_conv_out).reshape(T,B,C,N).contiguous()
        k2_conv_out = self.k2_lif(k2_conv_out)
        k2 = k2_conv_out.transpose(-1, -2).reshape(T, B, N, self.num_heads, C//self.num_heads).permute(0, 1, 3, 2, 4).contiguous()
        ...
        k = self.derive_high_order_k(k1, k2) * self.alpha + k1  # Core function, q.shape = qa.shape
        x = k.transpose(-2,-1) @ v
        x = (q @ x) * self.scale
...
class Spiking_Self_Attention(nn.Module):
    ...
    def forward(self, x):
        T, B, C, H, W = x.shape  # time  batch  channel  height  width
...
class HigherOrderSpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.hssa = Spiking_Higher_Order_Self_Attention(dim, num_heads)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MLP(in_features= dim, hidden_features=mlp_hidden_dim, drop=drop)
...
class spiking_transformer(nn.Module):
    def __init__(self,
        ...
        stage3 = nn.ModuleList([HigherOrderSpikingTransformer(
            dim=embed_dims, num_heads=num_heads[2], mlp_ratio=mlp_ratios, qkv_bias=qkv_bias,
            qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[j],
            norm_layer=norm_layer, sr_ratio=sr_ratios)
            for j in range(depths - 2)])
